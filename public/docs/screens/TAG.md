...
- **業務上の位置付け**: タグマスタの一覧・追加・更新・削除画面。収支とタグの対応は TRANSACTION_TAG で別管理。
...
- **タグ削除と TRANSACTION_TAG**: タグ行を削除しても TRANSACTION_TAG の当該 TAG_ID は本画面では削除しない（収支側で参照切れになる可能性は別仕様で考慮）。
