...
| **フッター** | その週の予定・実績それぞれについて「収入」「支出」「総合計」を表示する。 |
...
- 「予定」行: その週に表示した予定取引の収入合計・支出合計・総合計（収入－支出）。
- 「実績」行: その週に表示した実績取引の収入合計・支出合計・総合計。
- 振替は収入・支出のいずれにも含めず、表示上は金額のみリストに出す。フッターの総合計は「収入－支出」で算出する。
