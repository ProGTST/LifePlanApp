...
- **実績**: TRANDATE_FROM ＝ その日の行を件数・金額に含める。
...
- **日付セルの集計**: 月カレンダーの各日は、その日付に該当する取引のみを集計。表示項目の順序・ラベル・表示条件・空の日の扱い・クリック時の動作は **3.3 月カレンダー：日付ブロック内の表示仕様** を参照。実績は TRANDATE_FROM＝その日で集計。予定は頻度・間隔・繰り返しに従い「対象日」を算出し、対象日に件数・金額を集計する（getPlanOccurrenceDates / getCalendarDaySummary）。
