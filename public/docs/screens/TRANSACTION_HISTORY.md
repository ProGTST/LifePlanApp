...
| 一覧エリア | **一覧タブ**: テーブル（取引日、種別、予定/実績、カテゴリ、タグ、勘定、項目名、金額、メモ等）。ページネーションは表示位置を固定（スクロール時も維持）。**週カレンダー**: 選択月の週一覧と週ごとの取引リスト。**月カレンダー**: グリッド（日付セルに件数・金額サマリ）、フッターに月合計 |
...
| 日付 From | filterDateFrom | 日付 | — | YYYY-MM-DD | 実績は TRANDATE_FROM（未設定時は TRANDATE_TO）の日付で比較。予定は**予定発生日**（getPlanOccurrenceDates。完了予定日は考慮しない）のいずれかが範囲内に含まれるかで判定。 |
...
- **表示対象の取引**: (1) DLT_FLG が "1" でない、(2) ACCOUNT_ID_IN または ACCOUNT_ID_OUT が「参照可能な勘定」に含まれる、(3) 検索条件（予定/実績・種別・日付・カテゴリ・タグ・勘定・金額・フリーテキスト）を満たす。**日付条件と予定**: 予定取引は**予定発生日**（頻度・間隔・繰り返しに従う発生日。getPlanOccurrenceDates）のいずれかが filterDateFrom～filterDateTo の範囲内にあるもののみ。完了予定日のみの重なりでは抽出しない（transactionDataFilter.applyFilters）。月カレンダーはさらに (4) 取引日が選択年月に含まれるもののみ。**予定取引の日付条件**: 予定は日付範囲で絞る場合、**予定発生日**（頻度・間隔・繰り返しに基づく発生日。getPlanOccurrenceDates）のいずれかが範囲内に含まれるもののみ表示し、完了予定日のみの一致は考慮しない（transactionDataFilter.applyFilters）。
