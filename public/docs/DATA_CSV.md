...
| COMPLETED_PLANDATE | 完了予定日 | 予定取引のみ使用。一部の予定発生日を「完了扱い」にする日付をカンマ区切りで保持（例: `2026-01-01,2026-02-05`）。空の場合は未設定。検索条件で「完了」がOFFのとき、画面表示ではこの日付を発生日から除く。 |
...
| CARRYOVER_TOTAL | 繰越残高 | 初月は 0。2月目以降は「前月の CARRYOVER_TOTAL ＋ 当月の BALANCE_TOTAL」の累計（当該勘定・計画種別・年月順） |
